...
<div class="row text-center">
	...
	<div class="row">
		...
		@foreach($medias as $med)
				...
				<td> {{ $med->hits }}</td>	
				<td> {!! link_to('admin/mediaorder/'.$med->id.'/Down/'.$down.'/'.$med->id, '-',array('class'=>'btn btn-info')) !!}</td>
				<td> {!! link_to('admin/mediaorder/'.$med->id.'/Up/'.$up.'/'.$med->id, '+',array('class'=>'btn btn-info')) !!}</td>
				<td>
				<?php if($med->publish=='1'){?>
				{!!  link_to('admin/mediapub/'.$med->id.'/False/'.$med->id, 'Ok',array('class'=>'btn btn-success')) !!}
				<?php } else{ ?>
				{!! link_to('admin/mediapub/'.$med->id.'/True/'.$med->id, 'No',array('class'=>'btn btn-warning')) !!}
				<?php } ?>
				</td>
				<td> 
